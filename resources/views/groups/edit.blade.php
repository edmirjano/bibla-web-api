        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 sm:px-20 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ isset($group) ? route('group.update', $group->id) : route('group.store') }}" enctype="multipart/form-data">
                        @csrf
                        @if(isset($group))
                            @method('PUT')
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input id="name"
                                       ...
                                       type="text" name="name" value="{{ isset($group) ? $group->name : old('name') }}" required autofocus/>
                                @error('name')
                            ...
                            <div>
                                ...
                                <select id="book_id" name="book_id"
                                    ...
                                    @foreach($books as $book)
                                        <option value="{{ $book->id }}" {{ isset($group) && $group->book_id == $book->id ? 'selected' : '' }}>{{ $book->name }}</option>
                                    @endforeach
                                </select>
                                @error('book_id')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
